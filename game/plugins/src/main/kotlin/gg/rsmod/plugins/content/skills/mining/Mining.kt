...
//        val level = p.getSkills().getCurrentLevel(Skills.MINING)
    ...
    private fun canMine(p: Player, obj: GameObject, ore: OreType): Boolean {
        ...
        val pickaxe = PickAxeType.values.lastOrNull {
            p.getSkills()
                .getBaseLevel(Skills.MINING) >= it.level && (p.equipment.contains(it.item) || p.inventory.contains(it.item))
        }
        if (pickaxe == null) {
